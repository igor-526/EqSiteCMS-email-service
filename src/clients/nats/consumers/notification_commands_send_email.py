...
class NotificationCommandsSendEmailConsumer:
    def __init__(
        self,
        *,
        client: NatsJetstreamClient,
        settings: NatsSettings,
        handler: NotificationCommandSendEmailHandlerProtocol,
    ) -> None:
        self._client = client
    ...
    async def start(self) -> None:
        ...
        self._subscription = await self._client.jetstream.pull_subscribe(
            subject=self._settings.nats_subject_notification_commands_send_email,
            durable=self._settings.nats_consumer_notification_commands_send_email,
            stream=self._settings.nats_stream_notification_commands,
        )

        self._task = asyncio.create_task(
            self._consume(),
            name="notification-commands-send-email-consumer",
        )
