...
async def test_lifespan_closes_consumer_nats_database_and_metrics(
    ...
    monkeypatch.setattr(main.container, "notification_command_send_email_consumer", Mock(return_value=consumer))
...
async def test_nats_startup_failure_is_not_masked_by_metrics_cleanup(
    ...
    monkeypatch.setattr(main.container, "nats_client", Mock(return_value=nats))
    monkeypatch.setattr(main.container, "notification_command_send_email_consumer", Mock(return_value=Mock()))
    start_metrics = Mock()
